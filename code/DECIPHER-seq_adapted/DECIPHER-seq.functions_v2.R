# Adapted in May 2023 by Stephan Gruener, original code: https://github.com/lmurrow/DECIPHER-seq
# Main objective, to adapt for running with single cell type and for saving RAM
# e.g. replaced all seu_sub with seurat.object to not have two identical seurat objects loaded in RAM when analyzing only 1 cell type
# Therefore this function is not working properly when there is more than one cell type
# using mapply instead of mcmapply
# using lapply instead of mclapply
# changes are tracked via comments
...
iNMF_ksweep <- function(seurat.object, Type.thresh = 100, Sample.thresh = 10, Batch = TRUE, 
                        ...
                        max.cores = NULL, output.reps = FALSE, return.scale.data = T, results_path = "results/"){
  if (length(unique(seurat.object@meta.data$Type)) > 1){
    stop("This function was adapted to only work with one cell type. If you wish to run the function across different cell types, please use the original DECIPHER-seq function!")
  }  
  ...
  cell.types <- unique(seurat.object@meta.data[,"Type"])
  ...
  res <- lapply(cell.types, function(x){
    ...
    seurat.object <- subset(seurat.object, Type==x)
    if (Batch){
      if (length(table(seurat.object$Batch))>1){
        Batch.list <- SplitObject(seurat.object, split.by="Batch")
        Liger.setup=list()
      ...
      seurat.object <- AddMetaData(seurat.object, col.name = 'Batch', metadata = 'Batch')
      Liger <- createLiger(list(Batch1 = seurat.object@assays$RNA@counts))
    }
    
    #~~~~ added: remove Seurat objects
    rm(Batch.list)
    rm(seurat.object)
    gc()
    #~~~~# end
    ...
    minibatch_size <- min(table(seurat.object@meta.data$Batch))
    ...
    #~~~~ added: checking whether intermediate objects are available, if yes, load them
    k.min = 2
    files <- list.files(path = results_path, pattern = "LigerObject_R.*.rds")
    k.done <- str_replace(files, pattern = "LigerObject_R", replacement = "") %>% str_replace(pattern = ".rds", replacement = "") %>% as.numeric() %>% sort()
    if (length(files) > 0) {
      for (i in 1:length(k.done)) {
        Liger_list[[paste0("R", k.done[i])]] <- readRDS(file = paste0(results_path, "LigerObject_R", k.done[i], ".rds"))
      }
      k.min = k.done[length(k.done)] + 1
    }
    #~~~~# end
    
    if (k.min < k.max) { #~~~~ added this line
      for (k in c(k.min:k.max)){ #~~~~ exchanged 2:k.max for k.min:k.max
        print(paste0('Running K = ', k))
        reps.k = rep(k, n.reps)
        names(reps.k) = paste0("rep", 1:length(reps.k))
        Liger_list[[paste0("R", k)]] = mapply(LIGER.run, K = reps.k, Liger = c(Liger), 
        ...
        #~~~~ added: saving intermediate Liger objects
        saveRDS(Liger_list[[paste0("R", k)]], file = paste0(results_path, "LigerObject_", "R", k, ".rds"))
        #~~~~# end
    ...
    Consensus.results = lapply(Liger_list, function(x){
      gc() #~~~~ added
      W_list = lapply(x, '[[', 'W')
