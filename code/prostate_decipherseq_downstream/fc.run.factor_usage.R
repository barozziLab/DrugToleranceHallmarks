...
metadata_ordered <- metadata_full %>%
  mutate(class = factor(class, levels = c("treatment-naive", "first_line", "resistant", "second_line")))
...
prog_stats <- H.data.long %>%
  ...
  mutate(gene_program = as.numeric(str_remove(gene_program, "R22_Program"))) %>%
  ungroup()
...
w <- metadata_ordered %>% dplyr::filter(class == "resistant") %>% pull(sample)
baseline <- apply(mean_prog_usage_ordered[w,], 2, mean)
w <- metadata_ordered %>% dplyr::filter(class == "resistant" | class == "second_line") %>% pull(sample)
mean_prog_usage_ordered_sub[w,] <- t(t(mean_prog_usage_ordered[w,]) - baseline)
...
pdf(out_path, width = 8, height = 3)
draw(hm_mean_linear)
dev.off()

out_path <- paste0(out_folder, "/H.mean.heatmap.scaled.pdf")  
pdf(out_path, width = 8, height = 2.5)
draw(hm_mean_scaled)
dev.off()
...
pdf(out_path, width = 5, height = 2.5)
plot(plt_mean_bplot)
dev.off()
...
H.stats <- H.data.long %>%
  ...
  mutate(program = as.numeric(str_remove(program, "R22_Program")))

## Mutual information + AUC associations

coi <- colnames(H.stats)
coi <- coi[!coi %in% c("program", "program_value", "kit")] #excluding kit (all v3)

coi_auc <- coi[!coi %in% c("batch", "condition", "treatment", "class")]
...
lvls_feature <- c("class_treatment_naive", "class_acute", "class_first_line", "class_second_line", "class_resistant")
...
pdf(out_path, width = 6.5, height = 2.5)
plot(plt)
dev.off()
...
pdf(out_path, width = 6.5, height = 3)
plot(plt)
dev.off()
...
#re-plot subtracted heatmap, based on this order, and excluding the 2d
w_samples <- mean_prog_usage_sub_long %>% filter(!(class_first_line & grepl("2d", sample))) %>% pull(sample) %>% unique()
