## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##
## Script name: decipher_workflow_prostate.R
##
## Description: 
#~
## Run adapted DECIPHER-seq workflow on prostate data
##
## Authors: 
#~
...
so <- readRDS(file = "../prostate_seurat/so.rds")

#~~~~~~~~~~
so[['Sample']] <- "Sample_LNCaP"
so[['Type']] <- "LNCaP"
so[['Batch']] <- so[['kit']]
...
# The results for this step can be directly loaded
# NMF_results <- iNMF_ksweep(so, k.max = 40, max.cores = 1, Batch = FALSE, results_path = results_path)
# saveRDS(NMF_results, file = paste0(results_path, "NMF_results_prostate.rds"))
NMF_results <- readRDS("NMF_results_prostate.rds")
...
ggsave(paste0(results_path, "/lineplot.pdf"), lineplot, width = 6, height = 2)

NMF_results_atK <- mapply(FUN = NMF_results_opt_k, NMF_results, k.use, program_outlier_score, SIMPLIFY = F)
saveRDS(NMF_results_atK, file = paste0("prostate_decipherseq_downstream/", "NMF_results_atK.rds"))
