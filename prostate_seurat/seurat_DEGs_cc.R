...
so <- hallmark_module_score(hallmark = hallmark, 
                            so = so, 
                            ctrl_genes = 100, 
                            name_prefix = "prior_normalization_")
...
write_tsv(x = avg_expr, file = paste(outFolder, "/avg_expr_prostate.tsv", sep = ""))
...
write_tsv(x = metadata_so, file = paste(outFolder, "/so_metadata_prior_normalization.tsv", sep = ""))
